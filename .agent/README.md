...
.ideas/                          ← 내부용 기획 문서 (gitignore)
├── README.md                    #   라우터 — 현재 Phase 위치 + 파일별 편집 권한
├── current-status.md            #   현재 진행률 스냅샷
├── roadmap.md                   #   Phase별 로드맵
├── features.md                  #   13가지 기능 목록
└── technical-dependencies.md    #   기능 간 의존성 그래프

docs/                            ← 외부 공개용 완성 문서 (git 추적)
└── README.md                    #   배포된 기능 설명 (GitHub Pages 등 외부 노출)
```

---

## 📁 .ideas/ vs docs/ — 문서 역할 구분

| 항목 | `.ideas/` | `docs/` |
|------|-----------|---------|
| **대상** | 내부 (에이전트 + 유저) | 외부 (사용자, 기여자) |
| **내용** | 살아있는 기획·진척·설계 논의 | 완성된 기능 설명, 사용법 |
| **상태** | 항상 변하는 스냅샷 | 배포된 기능 기준으로 안정적 |
| **git 추적** | ❌ gitignore | ✅ 추적 |
| **편집 빈도** | 작업할 때마다 | 기능 완료 시점에만 |

### 에이전트 행동 규칙

**작업 중**
- `.ideas/` 자유롭게 읽고 편집
- 설계 논의, 진척 업데이트, 인계 메모 모두 `.ideas/`에

**기능 완료 시 (PR 생성 전)**
- `docs/`의 해당 기능 섹션 업데이트 의무
- 아직 `docs/`에 해당 항목이 없으면 새 섹션 추가
- "완성된 기능"만 `docs/`에 기재 — 미완성·진행 중은 기재 금지
